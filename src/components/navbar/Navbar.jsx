  ...
  return (
    <div className='w-screen h-[100px] z-10 bg-slate-50 fixed drop-shadow-lg' data-aos="fade-down">
          ...
          <div className='mx-2 m-auto text-gray-600'>
            <div className="max-w-lg w-full lg:max-w-xs">
              ...
              <form method="get" action="#" className="relative z-50">
                ...
                <input type="search" name="search" id="search" className="block w-full pl-10 pr-3 py-2 border-2 border-black rounded-md leading-5 bg-white text-gray-600 placeholder-gray-400 focus:outline-none focus:bg-white focus:text-gray-900 sm:text-sm transition duration-150 ease-in-out" placeholder="Search" />
              </form>
            </div>
          </div>
      ...
      <ul className={!nav ? 'hidden' : 'Absolute bg-slate-50 w-full px-8 md:hidden'} ref={menuRef} data-aos="fade-down">
        {MenuItems.map((item) => {
          return (
            <NavLink key={item.id} to={item.url} onClick={handleClose} className='flex p-5 py-2 border-b-2 border-zinc-300 w-full hover:bg-zinc-300 hover:rounded-md hover:border-[#d9d9d9]'>
              {item.title}
            </NavLink>
          )
        })}
