...
const Partner = () => {
  ...
  return (
    <div className='text-white px-5 lg:px-32 xl:px-40 2xl:px-52'>
        ...
        {/* start partners */}
        <div ref={slideRef} className="content-container grid md:divide-x md:divide-[#6E6E6E] md:grid-cols-2">
            <div className="partner-img w-full">
                <img className='m-auto h-[210px]' src={require('../../assets/img/partners/'+ PartnerItems[value].imgURL +'.svg')} alt="" />
            </div>
            <div className="flex flex-col justify-center md:items-start w-full h-full">
                <div className="partner-content text-left ml-0 md:ml-20">
                    <h4 className='text-[30px] font-semibold mb-5'>{PartnerItems[value].name}</h4>
                    <p>{PartnerItems[value].description}</p>
                </div>
            </div>
        </div>
        <div className='flex justify-end pb-20 my-10'>
            <button className='w-10 h-10 border rounded-full bg-white mx-2 drop-shadow-xl transition-all hover:bg-[#e6e6e6]' onClick={handleOnPrevClick}>
                <ChevronLeftIcon className='w-full h-full text-[#0D527C]'/>
            </button>

            <button className='w-10 h-10 border rounded-full bg-white ml-3 drop-shadow-xl transition-all hover:bg-[#e6e6e6]' onClick={handleOnNextClick}>
